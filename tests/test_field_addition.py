...
def test_add_field(source_size, target_size, pos="center", dtype=np.int16):
    ...
    if pos == "top_left":
        layout.add_field(source, target, 0, 0)
    elif pos == "bottom_right":
        layout.add_field(source, target, target_max_x, target_max_y)
    elif pos == "center":
        layout.add_field(source, target, target_center_x, target_center_y)
...
def test_add_field_speed(target_size, source_size, rounds=1000, target_x=5, target_y=5, dtype=np.int16):
    ...
    for i in range(0, rounds):
        layout.add_field(source_field, target_field, target_x, target_y)
    end = timer()
...
class TestFieldAddition(unittest.TestCase):
    """Tests for `qfieldlayout` package."""
